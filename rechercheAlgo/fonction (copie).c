...
int shortCut(Circuit pilote, Action action1[200])
{
	...
	/*		puts("------------Avant------------------\n");
		...
		for(i=j+1;i<9;i++){
			...
			if( current.x+vCurrent.vx+possible[i].vx < pilote.tx
					&&  current.y+vCurrent.vy+possible[i].vy < pilote.ty
					&& autour[possible[i].vy+1][possible[i].vx+1].x!=-1
					&& !vitesseNulle
					&& poids[possible[i].vy+1][possible[i].vx+1]!=1000 
					&& ((vCurrent.vx+possible[i].vx)*(vCurrent.vx+possible[i].vx) +(vCurrent.vy+possible[i].vy)*(vCurrent.vy+possible[i].vy)) <=25
					&&  verifD(vFutur,pilote.mapD,autour[possible[i].vy+1][possible[i].vx+1],pilote.tx,pilote.ty)) {
				trouver=1;
				break;
...
void direction(Vitesse mapD[][MAX],short map[][MAX],Position *shortP,Vitesse *vPos,int taille,int taillex,int tailley){
	...
	for(int c=0;c<taille;c++){
		...
		if(map[pos[4].y][pos[4].x]==1000) {debut=decx/2;decx=3*decx;}

		if(map[pos[5].y][pos[5].x]==1000) {debut=-decx;decx=0;}
		...
		for(int i=0;i<decy;i++){
			for(int j=debut;j<decx;j++){
				temp.x=x+j;
				temp.y=y+i;
